...
def conexao_usuario(chat_server, usuarios, usuario):
  ...
  welcome = 'Servidor diz: Bem-Vindo!'
  ...
  while str(bytes_recebidos, encoding='utf8') != '{quit}':
    if str(bytes_recebidos, encoding='utf8') != '':
...
def start_chat_server():
  ...
  try:
    ...
    while True:
      try: # Esse socket estará ligado a esse endereço
        chat_server.bind(endereco_para_escutar)
      except OSError: # Caso o socket esteja ocupado
        PORT = PORT + 1
        endereco_para_escutar = (HOST, PORT)
        continue
      else:
        break
    print("Servidor (" + str(PORT) + ") online: Escutando mensagens")
